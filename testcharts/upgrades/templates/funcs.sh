...
wait_ns() {
	set +e
	set +x
	while ! (kubectl get pods --namespace "$1" | gawk '{ if ((match($2, /^([0-9]+)\/([0-9]+)$/, c) && c[1] != c[2] && !match($3, /Completed/)) || !match($3, /STATUS|Completed|Running/)) { print ; exit 1 } }'); do
...
prepare_kubecf() {
	...
	cd kubecf
	# Be sure to work on a clean branch
